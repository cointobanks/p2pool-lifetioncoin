...
extra_includes = []
import p2pool.lifetioncoin
import p2pool.networks
extra_includes.extend('p2pool.networks.' + x for x in p2pool.networks.nets)
import p2pool.lifetioncoin.networks
extra_includes.extend('p2pool.lifetioncoin.networks.' + x for x in p2pool.lifetioncoin.networks.nets)
...
try:
    ...
    if im64:
        bundle = bundle + 2
    sys.argv[1:] = ['py2exe']
    setup(name='p2pool-lifetioncoin',
        version=version,
        description='Peer-to-peer Lifetioncoin mining pool',
        author='Forrest Voight',
        author_email='forrest@forre.st',
        url='https://github.com/cointobanks/p2pool-lifetioncoin/',
        data_files=[
            ('', ['README.md']),
...
dir_name = 'p2pool_lifetioncoin_win' + win + '_' + version
